      ...
      Object.keys(data.ratingSourcessByTeam).forEach((sourceId) => {
        ...
        data.pourKeys.forEach((ratingId) => {
          const rating = new ModelReportRatingHandle(),
            ...
            { tooltip, total } = this.getProjectTooltipFromList(
              item[1][ratingId]
            );
          rating.id = id;
          rating.name = name;
    ...
    this.reportsService
      ...
      .subscribe((data) => {
        const {
          ...
          pushIds,
          othersIds,
        } = ReportsService.GetReportTypesList(this.ratingsList);
        ...
        res.forEach((report) => {
          if (isNumber(report.reportUserId)) {
            this.calculateColumnInTable(
              this.TOTAL.GOLDEN_HOURS,
              report.goldenHours
            );
            this.calculateColumnInTable(
              this.TOTAL.SELF_POURING.UNCONTACTABLE,
              report.selfPouring.uncontactable
            );
            this.calculateColumnInTable(
              this.TOTAL.SELF_POURING.NOT_PICK_UP,
              report.selfPouring.notPickUp
            );
            this.calculateColumnInTable(
              this.TOTAL.SELF_POURING.NO_NEED,
              report.selfPouring.noNeed
            );
            this.calculateColumnInTable(
              this.TOTAL.SELF_POURING.HANG_UP,
              report.selfPouring.hangUp
            );
            this.calculateColumnInTable(
              this.TOTAL.SELF_POURING.TWO_SENTENCES,
              report.selfPouring.twoSentences
            );
            this.calculateColumnInTable(
              this.TOTAL.SELF_POURING.MORE_THAN_TWO_SENTENCES,
              report.selfPouring.moreThanTwoSentences
            );

            this.calculateColumnInTable(
              this.TOTAL.REGULAR_POURING.UNCONTACTABLE,
              report.regularPouring.uncontactable
            );
            this.calculateColumnInTable(
              this.TOTAL.REGULAR_POURING.NOT_PICK_UP,
              report.regularPouring.notPickUp
            );
            this.calculateColumnInTable(
              this.TOTAL.REGULAR_POURING.NO_NEED,
              report.regularPouring.noNeed
            );
            this.calculateColumnInTable(
              this.TOTAL.REGULAR_POURING.HANG_UP,
              report.regularPouring.hangUp
            );
            this.calculateColumnInTable(
              this.TOTAL.REGULAR_POURING.TWO_SENTENCES,
              report.regularPouring.twoSentences
            );
            this.calculateColumnInTable(
              this.TOTAL.REGULAR_POURING.MORE_THAN_TWO_SENTENCES,
              report.regularPouring.moreThanTwoSentences
            );

            this.calculateColumnInTable(
              this.TOTAL.PUSH_COUNT,
              report.pushCount
            );
            this.calculateColumnInTable(
              this.TOTAL.SUGGESSTION_COUNT,
              report.suggestionCount
            );
            this.calculateColumnInTable(
              this.TOTAL.EMAIL_COUNT,
              report.emailCount
            );
            this.calculateColumnInTable(
              this.TOTAL.FLIRTING_COUNT,
              report.flirtingCount
            );
          }
          ...
          this.sourcesList.forEach((source) => {
            ...
            if (report.ratingSources && report.ratingSources[source.id]) {
              ...
              const {
                  data: pourHandledData,
                  totalRatingPoints: pourRatingPoints,
                } = this.handleRatingGroups(
                  report.reportUserId,
                  source.id,
                  pourObj,
                  'pour'
                ),
                {
                  ...
                  totalRatingPoints: pushRatingPoints,
                } = this.handleRatingGroups(
                  report.reportUserId,
                  source.id,
                  pushObj,
                ...
                ),
                {
                  ...
                  totalRatingPoints: othersRatingPoints,
                } = this.handleRatingGroups(
                  report.reportUserId,
                  source.id,
                  othersObj,
            ...
            this.sourcesList.forEach((source) => {
              ...
              this.pourKeys.forEach((key) => {
                ...
                if (this.sourcesColumnsInTableMap.get(keyMap)) {
                  this.totalSourcesPour[source.id].pour.push(
                    this.sourcesColumnsInTableMap.get(keyMap).pour
                  );
                }
              });
              this.pushKeys.forEach((key) => {
                ...
                if (this.sourcesColumnsInTableMap.get(keyMap)) {
                  this.totalSourcesPush[source.id].push.push(
                    this.sourcesColumnsInTableMap.get(keyMap).push
                  );
                }
              });
              this.otherKeys.forEach((key) => {
                ...
                if (this.sourcesColumnsInTableMap.get(keyMap)) {
                  this.totalSourcesOthers[source.id].others.push(
                    this.sourcesColumnsInTableMap.get(keyMap).others
                  );
                }
              });
            });
        ...
        this.totalGoldenHours = this.getTotalColumnInTable(
          this.TOTAL.GOLDEN_HOURS
        );
        this.totalSelfPouringUncontactable = this.getTotalColumnInTable(
          this.TOTAL.SELF_POURING.UNCONTACTABLE
        );
        this.totalSelfPouringNotPickUp = this.getTotalColumnInTable(
          this.TOTAL.SELF_POURING.NOT_PICK_UP
        );
        this.totalSelfPouringNoNeed = this.getTotalColumnInTable(
          this.TOTAL.SELF_POURING.NO_NEED
        );
        this.totalSelfPouringHangUp = this.getTotalColumnInTable(
          this.TOTAL.SELF_POURING.HANG_UP
        );
        this.totalSelfPouringTwoSentences = this.getTotalColumnInTable(
          this.TOTAL.SELF_POURING.TWO_SENTENCES
        );
        this.totalSelfPouringMoreThanTwoSentences = this.getTotalColumnInTable(
          this.TOTAL.SELF_POURING.MORE_THAN_TWO_SENTENCES
        );
        this.totalAllSelfPouring =
          this.totalSelfPouringUncontactable +
          this.totalSelfPouringNotPickUp +
          this.totalSelfPouringNoNeed +
          this.totalSelfPouringHangUp +
          this.totalSelfPouringTwoSentences +
          this.totalSelfPouringMoreThanTwoSentences;
        this.totalRegularPouringUncontactable = this.getTotalColumnInTable(
          this.TOTAL.REGULAR_POURING.UNCONTACTABLE
        );
        this.totalRegularPouringNotPickUp = this.getTotalColumnInTable(
          this.TOTAL.REGULAR_POURING.NOT_PICK_UP
        );
        this.totalRegularPouringNoNeed = this.getTotalColumnInTable(
          this.TOTAL.REGULAR_POURING.NO_NEED
        );
        this.totalRegularPouringHangUp = this.getTotalColumnInTable(
          this.TOTAL.REGULAR_POURING.HANG_UP
        );
        this.totalRegularPouringTwoSentences = this.getTotalColumnInTable(
          this.TOTAL.REGULAR_POURING.TWO_SENTENCES
        );
        this.totalRegularPouringMoreThanTwoSentences = this.getTotalColumnInTable(
          this.TOTAL.REGULAR_POURING.MORE_THAN_TWO_SENTENCES
        );
        this.totalAllRegularPouring =
          this.totalRegularPouringUncontactable +
          this.totalRegularPouringNotPickUp +
          this.totalRegularPouringNoNeed +
          this.totalRegularPouringHangUp +
          this.totalRegularPouringTwoSentences +
          this.totalRegularPouringMoreThanTwoSentences;
        this.totalPushCount = this.getTotalColumnInTable(this.TOTAL.PUSH_COUNT);
        this.totalSuggesstionCount = this.getTotalColumnInTable(
          this.TOTAL.SUGGESSTION_COUNT
        );
        this.totalEmailCount = this.getTotalColumnInTable(
          this.TOTAL.EMAIL_COUNT
        );
        this.totalFlirtingCount = this.getTotalColumnInTable(
          this.TOTAL.FLIRTING_COUNT
        );
  ...
  private getProjectTooltipFromList(data: any[]) {
    ...
    data.forEach((item) => {
      if (item[0] === 0) {
        ...
        const founded = this.projectsList.find(
          (project) => project.id === item[0]
        );
        if (founded) {
