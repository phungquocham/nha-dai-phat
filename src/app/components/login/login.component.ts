...
export class LoginComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.form = this.fb.group({
      ...
      'verifyCode': [null, Validators.compose([Validators.required, Validators.pattern(REGEX.NUMBER_LENGTH_6)])]
    });
  }
  ...
  private snackbarError(message: string) {
    this.snackbar.open({ type: SNACKBAR.TYPE.ERROR, message: message });
  }
  ...
  onSubmit(): void {
    if (this.form.controls['phone'].valid) {
      ...
      this.authService.signInWithPhoneNumber({
