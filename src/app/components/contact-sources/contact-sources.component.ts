...
export class ContactSourcesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  setValueFormControl(value) {
    this.searchFormControl.setValue(value, {emitEvent: false, onlySelf: true});
    if (!value) {
  ...
  applyFilter() {
    this.searchChange.pipe(debounceTime(500), distinctUntilChanged()).subscribe(key => {
      if (key !== null) {
        this.initialFilter();
  ...
  setQueryString = (params: Params) => {
    params.q ? this.setValueFormControl(params.q) : this.setValueFormControl(null);
  }
  ...
  getList() {
    ...
    this.contactSourceService.getList(this.filterData).pipe(takeUntil(this.ngUnsubscribe)).subscribe(response => {
      response.items.forEach(element => {
        element.lastAssignedDate = element.lastAssignedDate ? Utils.transform(element.lastAssignedDate) : '';
      });
  ...
  routeRedirect() {
    this.router.navigateByUrl(`${Routing.CONTACT_SOURCES}${Utils.encodeQueryData(this.filterData)}`);
  }
  ...
  onDelete(id) {
    this.dialog.open({
      ...
      if (res === CONFIRM.OK) {
        this.contactSourceService.delete(id).pipe(takeUntil(this.ngUnsubscribe)).subscribe(response => {
              this.snackbar.open({message: 'Xóa thành công!', type: 'SUCCESS'});
              this.getList();
            },
            err => {
              if (err.status === 404) {
                this.snackbar.open({message: 'Không xóa được.', type: 'ERROR'});
              }
            },
